 ...
 *   rows: [{ u, name, color, isWinner, rank,
 ...
 */
function describe(gameType, config, pp, isWinner) {
...
function highlightsFor(gameType, config, rows, perPlayer) {
  ...
  if (gameType === "x01") {
    ...
    best("Best hit rate", (pp) => (dartsThrown(pp) ? ((pp.targetsHit || 0) / dartsThrown(pp)) * 100 : 0), (v) => `${Math.round(v)}%`);
 ...
 */
export function buildSummary({ match, game, eloBefore, eloAfter, colors }) {
