 ...
 * and equal scores share a place. Captured on each result row as
 * `stats.place` so multiplayer standings survive without re-deriving.
 */
export function finishPlaces({ gameType, config = {}, players = [], winner, perPlayer = {} }) {
  ...
  scored.forEach((r, i) => {
    ...
    out[r.u] = i > 0 && !prev.won && !r.won && prev.score === r.score ? out[prev.u] : i + 1;
  });
  return out;
}
