...
import { MAX_PLANS } from "../trainingPlans.js";
...
/**
 * The caller's saved training plans (RLS: own rows only): titles and the
 * limit, so Blackbird AI knows whether a new plan can be made. Null when
 * plans aren't set up or the read fails; the chat then doesn't mention them.
 */
async function loadPlans(sb) {
  const { data, error } = await sb.from("training_plans").select("created_at, title:definition->>title").order("created_at", { ascending: true });
  if (error) return null;
  const saved = (data || []).map((r) => r.title).filter(Boolean);
  return { saved: saved.length, limit: MAX_PLANS, titles: saved, canCreate: saved.length < MAX_PLANS };
}
...
export async function buildServerSummary(sb, { userId, now = new Date() }) {
  ...
  const [own, circle, follows, events, plans] = await Promise.all([
  ...
  if (plans) summary.trainingPlans = plans;
