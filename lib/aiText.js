...
export const IDENTITY_REPLY =
  "I'm **Merlin**, the darts coach built into Blackbird. I read your logged games, so ask me about your form, checkouts, rivals, achievements or what to practice next.";
...
/**
 * Backstop for replies: any provider or model name becomes "Merlin" (and
 * the assistant's old name, "Blackbird AI", does too).
 * Fenced blocks (charts, follow-ups, actions) are left untouched.
 */
export function scrubIdentity(text) {
  return String(text || "")
    .split(/(```[\s\S]*?```)/g)
    .map((part) => (part.startsWith("```") ? part : part.replace(MODEL_NAMES, "Merlin").replace(COMPANY_NAMES, "Blackbird").replace(/\bBlackbird AI\b/g, "Merlin").replace(/Merlin(?:[ ,/]+(?:by|from|and)?\s*Merlin)+/gi, "Merlin")))
    .join("");
}
