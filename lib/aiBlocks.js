...
import { GOALS, SESSION_MINUTES, PLAN_BOUNDS } from "./trainingPlans.js";
 ...
 *                 or { type: "plan", goal, minutes, perWeek, weeks, note }:
 *                 opens Create With Merlin pre-filled (the plan itself is
 *                 drafted, validated and saved by /api/plans, never here)
...
/** One action, validated and normalized, or null. */
export function validateAction(a) {
  if (!a || typeof a !== "object") return null;
  const label = (s) => String(s || "").trim().slice(0, 44);
  if (a.type === "plan") {
    const goal = GOALS.some((g) => g.id === a.goal) ? a.goal : null;
    ...
    const plan = {
      type: "plan",
      goal,
      minutes: nearest(a.minutes, SESSION_MINUTES, 30),
      perWeek: int(a.perWeek, PLAN_BOUNDS.perWeek, 3),
      weeks: int(a.weeks, PLAN_BOUNDS.weeks, 2),
      note: String(a.note || "").replace(/\s+/g, " ").trim().slice(0, 140),
      label: label(a.label) || "Build This Plan",
    };
    // a plan can't have more sessions than the validator allows
    if (plan.perWeek * plan.weeks > PLAN_BOUNDS.sessions[1]) plan.weeks = Math.floor(PLAN_BOUNDS.sessions[1] / plan.perWeek);
    return plan;
  }
  if (a.type === "bot") {
