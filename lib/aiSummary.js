...
import { replayX01Log, checkoutRange } from "./x01log.js";
...
/** Per-game derived numbers for one competitive row. */
export function describeGame(r, me) {
  ...
  if (r.gameType === "x01") {
    ...
    const rp = replayX01Log(pp.darts, start, doubleOut);
    const darts = pp.dartsThrown || rp.dartsThrown || 0;
    const points = pp.pointsScored || rp.pointsScored || 0;
    return {
      ...base,
      start,
      doubleOut,
      ...
      first9Avg: rp.first9 != null ? round(rp.first9 / 3, 1) : null,
      highestTurn: pp.highestTurn || rp.highestTurn || 0,
      checkout: r.winner === me ? pp.checkout || rp.checkoutScore || 0 : 0,
      checkoutChances: rp.valid ? rp.chances : null,
      checkoutHit: rp.valid ? rp.checkoutHit : r.winner === me ? 1 : 0,
      busts: rp.valid ? rp.busts : null,
      tons: rp.valid ? rp.tons : null,
      one80s: rp.valid ? rp.one80s : null,
      _rp: rp,
    };
  }
  if (r.gameType === "cricket") {
    return {
      ...
      marks: pp.marks || 0,
      rounds: pp.rounds || 0,
      mpr: pp.rounds ? round((pp.marks || 0) / pp.rounds, 2) : null,
      points: pp.pointsScored || 0,
    };
  }
  if (r.gameType === "baseball") return { ...base, runs: pp.runs || 0 };
  const extra = {};
  for (const k of ["dartsThrown", "shanghai", "halves", "resetsDealt", "finalScore"]) if (pp[k] != null) extra[k] = pp[k];
  return { ...base, ...extra };
}
...
function bucketize(games, unit, limit) {
  const map = new Map();
...
function seriesFrom(buckets, metric, nKey = "games") {
  const out = [];
  buckets.forEach((b) => {
...
function gameSeries(games, pick) {
  const out = [];
  for (const g of games) {
    const y = pick(g);
    if (y == null) continue;
...
export function buildMySummary({ me, stats, elo, results, practice, players, now = new Date() }) {
  const s = stats?.[me];
  ...
  for (const r of rows) {
    for (const o of r.opponents || []) {
      if (!h2h[o]) h2h[o] = { opponent: o, wins: 0, losses: 0, games: 0, lastPlayed: null };
      const h = h2h[o];
      h.games++;
      if (r.winner === me) h.wins++;
      else if (r.winner === o) h.losses++;
      h.lastPlayed = day(r.completedAt);
    }
  }
  const headToHead = Object.values(h2h)
    .map((h) => ({ ...h, winPct: pct(h.wins, h.games), opponentElo: Math.round(elo[h.opponent] || BASE_ELO) }))
