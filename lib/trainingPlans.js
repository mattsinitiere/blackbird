/**
 * Training plans: the catalogue of drills a plan may contain, strict
 ...
 * normalised copy (labels and minutes computed here, never trusted from
...
/** True when there isn't enough history for a personalised plan. */
export function needsStarter(baseline) {
  return (baseline?.games || 0) < STARTER_MIN_GAMES;
}

/** A clearly labelled assessment plan for players with little history. */
export function starterPlan() {
  const v = validatePlanDefinition({
    ...
    perWeek: 3,
    starter: true,
    why: "There isn't enough recorded play yet for a personalised plan. These three sessions measure your scoring, doubles and finishing, so your next plan can be built on real numbers.",
    sessions: [
