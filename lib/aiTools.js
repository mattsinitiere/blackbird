...
import { dayKey, weekKey, monthKey, dayWindow } from "./data/tz.js";
...
export const TOOL_DEFS = [
  ...
  },
  {
    name: "dart_heatmap",
    description:
      "Which beds a player's logged darts hit: counts per board number and ring (single, double, triple, bull), from the beds tapped when scoring. " +
...
/** A YYYY-MM-DD from the model, or null. */
...
/**
 * [from, to) instants for a tool's from/to days, as local days in the
 * reporting timezone (lib/data/tz.js), both ends inclusive of whole days.
 */
export function toolWindow(from, to) {
  const w = dayWindow(isoDay(from), isoDay(to));
  return { from: w.from ? w.from.getTime() : null, to: w.to ? w.to.getTime() : null };
}
 ...
 */
export function createToolRunner({ rows, players = [], me, now = new Date(), shared = null }) {
  const all = (rows || []).slice().sort((a, b) => new Date(a.completedAt) - new Date(b.completedAt));
  // `shared` keeps series/heatmap ids unique across several runners (the
  // scoped runner builds one per tool call over freshly fetched rows)
  const reg = shared || newRegistry();
  const { series, heatmaps } = reg;
  ...
  const filterRows = ({ player, gameType, from, to, opponent, result, includePractice }) => {
    ...
    const { from: f, to: t } = toolWindow(from, to);
    return {
      u,
      opp,
      list: all.filter((r) => {
...
/** Where series and heatmaps computed by tools are kept under short ids. */
export function newRegistry() {
  return { series: {}, heatmaps: {}, sn: 0, hn: 0 };
}
...
// weeks start on Monday and months are calendar months, both in the
// reporting timezone (they used to be UTC days and UTC months)
function periodKey(date, groupBy) {
  return groupBy === "week" ? weekKey(date) : monthKey(date);
}
