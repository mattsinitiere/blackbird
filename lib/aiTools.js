...
/**
 * Not offered to the model yet: darts are logged by the bed a player taps,
 * not where the dart actually landed, so a board heatmap would look more
 * precise than the data is. Add it to TOOL_DEFS once positional (x/y) data
 * is captured; the tool, the chart type and DartHeatmap are ready.
 */
export const HEATMAP_TOOL_DEF = {
    name: "dart_heatmap",
    description: "Where a player's darts landed: hit counts per board number and ring (single, double, treble, bull) plus misses, from logged darts. Returns a heatmap id to use in a chart block of type heatmap.",
    parameters: { type: "object", properties: { ...common } },
  };
...
export function createToolRunner({ rows, players = [], me, now = new Date() }) {
  ...
  const tools = {
    ...
    dart_heatmap(a = {}) {
      ...
      const id = `h${++hn}`;
      heatmaps[id] = { counts, darts, misses, player: u };
      const top = Object.entries(counts)
        .map(([n, c]) => ({ segment: n, hits: c.S + c.D + c.T, ...c }))
        .sort((x, y) => y.hits - x.hits)
        .slice(0, 8);
      return { id, player: u, darts, misses, missPct: round((misses / darts) * 100, 1), topSegments: top };
    },
  };
