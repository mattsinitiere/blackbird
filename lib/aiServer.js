...
export function friendlyError(e) {
  ...
  if (/_API_KEY is not set|Unknown AI_PROVIDER/.test(msg)) return { status: 503, error: "Merlin isn't switched on yet. The site owner needs to add an AI provider key." };
  if (e?.config) return { status: 503, error: "Merlin is misconfigured on the server, so it can't answer right now. The site owner has been told what to fix." };
  if (e?.quota) return { status: 429, error: "Merlin has hit its usage limit for now. Try again in a minute (or tomorrow if it keeps happening)." };
  if (e?.name === "TimeoutError" || e?.name === "AbortError") return { status: 504, error: "Merlin took too long to answer. Try again." };
