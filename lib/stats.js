import { X01_TARGETS, BASE_ELO, ELO_K } from "./constants.js";
...
export function computeStats(results) {
  ...
  const init = () => ({
    ...
    cricket: { games: 0, wins: 0, marks: 0, rounds: 0, points: 0, bestMpr: 0, perNumber: {}, loggedGames: 0, loggedDarts: 0, misses: 0 },
    baseball: { games: 0, wins: 0, runs: 0 },
    aroundTheClock: { games: 0, wins: 0, totalDarts: 0, dartsInWins: 0 },
  ...
  for (const r of results) {
    ...
    if (r.gameType === "x01") {
      ...
      if (won) {
        // stats v2 best-of matches carry per-leg darts; older rows are one leg
        const wonLegs = Array.isArray(pp.legs) ? pp.legs.filter((l) => l.w === u && l.d > 0).map((l) => l.d) : [];
        const leg = wonLegs.length ? Math.min(...wonLegs) : pp.dartsThrown;
        if (leg && (x.bestLeg == null || leg < x.bestLeg)) x.bestLeg = leg;
      ...
      if (Array.isArray(pp.darts)) {
        // where the darts landed: only stats v2 rows logged misses, so the
        // all-darts denominator counts those games alone
        const logged = pp.v >= 2;
        if (logged) {
          c.loggedGames++;
          c.loggedDarts += pp.darts.length;
        }
      ...
      if (won) s[u].aroundTheClock.dartsInWins += pp.dartsThrown || 0;
  ...
  for (const u in s) {
    ...
    p.aroundTheClock.avgDarts = p.aroundTheClock.wins ? p.aroundTheClock.dartsInWins / p.aroundTheClock.wins : 0;
    p.cricket.missPct = p.cricket.loggedDarts ? (p.cricket.misses / p.cricket.loggedDarts) * 100 : null;
...
/**
 * One rivalry in full, from `me`'s own ranked rows: the overall record and
 * the record per game type, the current streak and the last five meetings.
 * `losses` are games `opp` won; `otherWinner` are multiplayer games a third
 * player won (counted in `games`, in neither wins nor losses). Pure.
 */
export function rivalry(results, me, opp) {
  const rows = (results || [])
    .filter((r) => r.username === me && r.result !== "practice" && (r.opponents || []).includes(opp))
    .sort((a, b) => new Date(a.completedAt) - new Date(b.completedAt));
  const blank = () => ({ games: 0, wins: 0, losses: 0, otherWinner: 0 });
  const total = blank();
  const byGameType = {};
  const meetings = [];
  for (const r of rows) {
    const out = r.winner === me ? "W" : r.winner === opp ? "L" : "O";
    for (const t of [total, (byGameType[r.gameType] = byGameType[r.gameType] || blank())]) {
      t.games++;
      if (out === "W") t.wins++;
      else if (out === "L") t.losses++;
      else t.otherWinner++;
    }
    meetings.push({ result: out, gameType: r.gameType, winner: r.winner, date: r.completedAt, gameId: r.gameId, players: (r.opponents || []).length + 1 });
  }
  // streak over the head-to-head decisions only (a third player's win breaks nothing)
  let streak = null;
  for (let i = meetings.length - 1; i >= 0; i--) {
    const m = meetings[i];
    if (m.result === "O") continue;
    if (!streak) streak = { result: m.result, count: 1 };
    else if (streak.result === m.result) streak.count++;
    else break;
  }
  return {
    opponent: opp,
    ...total,
    winPct: total.games ? Math.round((total.wins / total.games) * 1000) / 10 : null,
    byGameType,
    streak,
    last5: meetings.slice(-5).reverse(),
    firstPlayed: meetings[0]?.date || null,
    lastPlayed: meetings[meetings.length - 1]?.date || null,
  };
}
