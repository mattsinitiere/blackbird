 ...
 * (ties: triple before double before single, then higher number). The
...
/** Beds a setup visit aims at when no finish is on: singles and triples 1–20. */
...
function checkoutAdvice(ranked, { darts, dOut, preferredDouble, evidence }) {
  ...
  if (basis === "data") {
    ...
    const tail = setups && rob === setups && choice.route.slice(0, -1).some((d) => d.mult !== 1) ? "; a single instead of the triple still leaves a finish" : "";
    reason = dOut
