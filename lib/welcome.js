...
function badgeInsight(badges) {
  ...
  const p = b.progress;
  let text;
  if (p.kind === "streak") text = `${p.value} ${p.value === 1 ? "win" : "wins"} in a row. ${p.target - p.value} more for ${b.title}.`;
  else if (p.kind === "best") text = `Your best is ${p.value}. Hit ${p.target} for ${b.title}.`;
  else text = `${p.target - p.value} to go for ${b.title} (${p.value}/${p.target})`;
  return { kind: "badge", text, badge: b };
}
