...
export function matchFeed(results, user) {
  ...
  const items = rows.map((row, i) => {
    ...
    const primary = d.primary && d.primary.label && !(d.primary.label === "legs" && row.stats?.legsWon == null) ? d.primary : null;
    return {
      kind: "match",
      ...
      primary,
      // never repeat the headline figure (No-score cricket's headline is MPR)
      figures: (d.stats || [])
        .filter((s) => s.value !== "" && s.value != null && !(s.label === "darts" && !s.value))
        .filter((s) => !primary || s.label.toLowerCase() !== primary.label.toLowerCase())
        .slice(0, 3),
      eloAfter,
      eloDelta,
