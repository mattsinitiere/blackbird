...
/** The bot a result just unlocked: locked in `before`, open in `after`. */
export function newlyUnlockedBot(before, after) {
  const was = new Map((before || []).map((l) => [l.bot.id, l.unlocked]));
  const hit = (after || []).find((l) => l.unlocked && was.get(l.bot.id) === false);
