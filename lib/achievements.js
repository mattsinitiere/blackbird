...
export const ACHIEVEMENTS = [
  ...
  // ---- scoring ----
  { id: "ton_up", category: "Scoring", title: "Ton Up", description: "Score 100 or more in a visit.", icon: "gauge", test: (c) => firstBadge(c.games, (g) => x01(g) && ((m(g, "tons") || 0) >= 1 || (g.row.stats?.highestTurn || 0) >= 100)) },
  { id: "ton_40", category: "Scoring", title: "140 Club", description: "Score 140 or more in a visit.", icon: "flame", test: (c) => firstBadge(c.games, (g) => x01(g) && ((m(g, "ton40s") || 0) >= 1 || (g.row.stats?.highestTurn || 0) >= 140)) },
  { id: "one_eighty", category: "Scoring", title: "Maximum", description: "Hit a 180.", icon: "burst", test: (c) => firstBadge(c.games, (g) => one80s(g) >= 1) },
  ...
  { id: "avg_60", category: "Scoring", title: "Sixty Average", description: "Average 60+ per three darts over a whole game (at least 9 darts).", icon: "bars", test: (c) => maxBadge(c.games, (g) => (x01(g) && (g.row.stats?.dartsThrown || 0) >= 9 ? m(g, "threeDartAvg") : 0), 60) },
  // ---- finishing ----
  { id: "first_checkout", category: "Finishing", title: "Closer", description: "Take out a leg.", icon: "lock", test: (c) => firstBadge(c.games, (g) => x01(g) && g.won && (g.row.stats?.checkout || 0) > 0) },
  { id: "checkout_100", category: "Finishing", title: "Ton-Plus Finish", description: "Check out from 100 or more.", icon: "impact", test: (c) => maxBadge(c.games, (g) => (x01(g) && g.won ? g.row.stats?.checkout || 0 : 0), 100) },
  { id: "checkout_170", category: "Finishing", title: "Big Fish", description: "Check out 170.", icon: "fish", test: (c) => firstBadge(c.games, (g) => x01(g) && g.won && g.row.stats?.checkout === 170) },
  { id: "bull_finish", category: "Finishing", title: "Bullseye", description: "Finish a leg on the bull.", icon: "crosshair", test: (c) => firstBadge(c.games, (g) => { if (!x01(g) || !g.won) return false; const d = g.row.stats?.darts; const last = Array.isArray(d) && d.length ? d[d.length - 1] : null; return !!last && last.n === 25 && last.mult === 2; }) },
  { id: "short_leg", category: "Finishing", title: "Eighteen Darter", description: "Win a 501 leg in 18 darts or fewer.", icon: "stopwatch", test: (c) => firstBadge(c.games, (g) => x01(g) && g.won && (g.row.config?.startScore || 0) === 501 && (m(g, "bestLeg") || g.row.stats?.dartsThrown || 999) <= 18) },
  // ---- cricket ----
  { id: "cricket_first_win", category: "Cricket", title: "Closed Out", description: "Win a game of Cricket.", icon: "wicket", test: (c) => firstBadge(c.games, (g) => cricket(g) && g.won) },
  { id: "mpr_3", category: "Cricket", title: "Three Marks", description: "Average 3+ marks per round over a Cricket game (at least 5 rounds).", icon: "triple", test: (c) => maxBadge(c.games, (g) => (cricket(g) && (g.row.stats?.rounds || 0) >= 5 ? m(g, "mpr") : 0), 3) },
  // ---- streaks ----
  ...
  { id: "crew_5", category: "Social", title: "Crew", description: "Follow five players.", icon: "users", selfOnly: true, test: (c) => ({ earnedAt: nthDate(c.social?.following, 5), progress: { value: Math.min(c.social?.following?.length || 0, 5), target: 5 } }) },
  { id: "popular_5", category: "Social", title: "Crowd Favourite", description: "Be followed by five players.", icon: "heart", selfOnly: true, test: (c) => ({ earnedAt: nthDate(c.social?.followers, 5), progress: { value: Math.min(c.social?.followers?.length || 0, 5), target: 5 } }) },
  // ---- practice ----
  ...
  { id: "ladder_top", category: "Practice", title: "Top of the Ladder", description: `Beat ${BOTS[BOTS.length - 1]?.name || "the top bot"}, the last bot on the ladder.`, icon: "ladder", test: (c) => { const pr = computePractice(c.practiceRows, c.me, c.now); const level = pr.bots.ladder.filter((l) => l.unlocked).length; const hit = c.drills.find((g) => g.won && (g.row.opponents || []).includes(topBot)); return { earnedAt: hit ? day(hit.date) : null, progress: { value: level, target: BOTS.length } }; } },
