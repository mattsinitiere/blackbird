...
function buildContext({ me, results, practice, social, now }) {
  ...
  // customized each part of their profile); only present for yourself
...
export const ACHIEVEMENTS = [
  ...
  { id: "popular_5", category: "Social", title: "Crowd Favorite", description: "Be followed by five players.", icon: "heart", selfOnly: true, test: (c) => ({ earnedAt: nthDate(c.social?.followers, 5), progress: { value: Math.min(c.social?.followers?.length || 0, 5), target: 5, kind: "count" } }) },
  // ---- practice ----
  ...
  { id: "addict", category: "Dedication", title: "Addict", description: "Play 50 games in one day (practice counts). Maybe take a break?", icon: "burst", tier: 3, test: (c) => { const n = {}; let best = 0; let earnedAt = null; for (const g of c.everything) { const k = localDay(g.date); n[k] = (n[k] || 0) + 1; if (n[k] > best) best = n[k]; if (n[k] === 50 && !earnedAt) earnedAt = day(g.date); } return { earnedAt, progress: { value: Math.min(best, 50), target: 50, kind: "best" } }; } },
  // ---- profile ----
  { id: "fresh_paint", category: "Profile", title: "Fresh Paint", description: "Pick your own player color.", icon: "palette", selfOnly: true, test: (c) => ({ earnedAt: c.activity?.profile?.color || null }) },
  ...
  { id: "grand_slam", category: "Game Modes", title: "Grand Slam", description: "Score 9 runs in one Baseball inning (three triples).", icon: "baseball", tier: 3, test: (c) => maxBadge(c.games, (g) => (g.row.gameType === "baseball" ? m(g, "biggestInning") : 0), 9) },
  ...
  { id: "nine_marks", category: "Game Modes", title: "Nine-Mark Round", description: "Score 9 marks in one Cricket round (three triples).", icon: "triple", tier: 3, test: (c) => maxBadge(c.games, (g) => (cricket(g) ? m(g, "bestRound") || Math.max(0, ...(g.row.stats?.roundMarks || [])) : 0), 9) },
  { id: "shanghai_win", category: "Game Modes", title: "Shanghai!", description: "Win a ranked Shanghai game with a Shanghai (single, double and triple in one round).", icon: "burst", tier: 2, test: (c) => firstBadge(c.games, (g) => g.row.gameType === "shanghai" && g.won && !!g.row.stats?.shanghai) },
