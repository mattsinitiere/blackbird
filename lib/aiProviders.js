...
async function openaiStep(cfg, args) {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      data = await post(url, { Authorization: `Bearer ${cfg.key}` }, body, cfg.provider);
      break;
    } catch (e) {
      const param = e.status === 400 ? unsupportedParam(e.message) : null;
      if (attempt > 0 || !param || !(param in body) || param === "messages" || param === "model") throw e;
      body = { ...body };
      delete body[param];
  ...
  const toolCalls = (msg.tool_calls || []).map((c) => {
    let args2 = {};
    try {
      args2 = JSON.parse(c.function?.arguments || "{}");
    } catch {}
    return { id: c.id, name: c.function?.name, args: args2 };
  });
