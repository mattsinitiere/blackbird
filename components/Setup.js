...
export default function Setup({ players, onStart, back, me, playerColors, initial = null, onOpenFriends = null, onOpenBots = null }) {
  ...
  const [startScore, setStartScore] = useState([301, 501, 701].includes(cfg.startScore) ? cfg.startScore : 501);
  const [doubleOut, setDoubleOut] = useState(cfg.doubleOut !== false);
  const [legs, setLegs] = useState([1, 3, 5, 7].includes(cfg.legs) ? cfg.legs : 1);
  ...
  const [checkoutCount, setCheckoutCount] = useState([5, 10, 20].includes(cfg.count) ? cfg.count : 10);
  const [scoringTarget, setScoringTarget] = useState(SCORING_TARGETS.includes(cfg.target) ? cfg.target : 20);
  const [scoringTurns, setScoringTurns] = useState(SCORING_TURNS.includes(cfg.turns) ? cfg.turns : 10);
